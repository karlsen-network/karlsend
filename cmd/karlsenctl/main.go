...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		karlsendMessage, err := client.Post(&protowire.KarlsendMessage{Payload: &protowire.KarlsendMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := karlsendMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	karlsendMessage := &protowire.KarlsendMessage{}
	err := protojson.Unmarshal([]byte(response), karlsendMessage)
	if err != nil {
	...
	return marshalOptions.Format(karlsendMessage)
}
