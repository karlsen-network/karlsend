...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libkarlsenwallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libkarlsenwallet.UTXO{
	...
	return libkarlsenwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by karlsenwallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libkarlsenwallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return libkarlsenwallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libkarlsenwallet.UTXO, requiredAmount uint64, feeRate float64) (
	additionalUTXOs []*libkarlsenwallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libkarlsenwallet.UTXO{
