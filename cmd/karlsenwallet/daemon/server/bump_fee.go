...
func (s *server) BumpFee(_ context.Context, request *pb.BumpFeeRequest) (*pb.BumpFeeResponse, error) {
	...
	if len(domainTx.Outputs) == 0 || len(domainTx.Outputs) > 2 {
		return nil, errors.Errorf("karlsenwallet supports only transactions with 1 or 2 outputs in transaction %s, but this transaction got %d", request.TxID, len(domainTx.Outputs))
	}
