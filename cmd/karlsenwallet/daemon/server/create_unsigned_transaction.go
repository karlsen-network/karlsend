...
// By having at least 10KLS in the change output we make sure that the storage mass charged for change is
// at most 1000 gram. Generally, if the payment is above 10KLS as well, the resulting storage mass will be
// in the order of magnitude of compute mass and wil not incur additional charges.
// Additionally, every transaction with send value > ~0.1 KLS should succeed (at most ~99K storage mass for payment
// output, thus overall lower than standard mass upper bound which is 100K gram)
const minChangeTarget = constants.SompiPerKarlsen * 10
...
func (s *server) calculateFeeLimits(requestFeePolicy *pb.FeePolicy) (feeRate float64, maxFee uint64, err error) {
	...
	case nil:
		...
		feeRate = estimate.Estimate.NormalBuckets[0].Feerate
		// Default to a bound of max 1 KLS as fee
		maxFee = constants.SompiPerKarlsen
	}
...
func (s *server) estimateFee(selectedUTXOs []*libkarlsenwallet.UTXO, feeRate float64, maxFee uint64, recipientValue uint64) (uint64, error) {
	...
	var mockPayments []*libkarlsenwallet.Payment
	if totalValue > recipientValue {
		mockPayments = []*libkarlsenwallet.Payment{
			{
				Address: fakeAddr,
	...
	mockTx, err := libkarlsenwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
...
func (s *server) estimateFeePerInput(feeRate float64) (uint64, error) {
	mockUTXO := &libkarlsenwallet.UTXO{
	...
	mockTx, err := libkarlsenwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, []*libkarlsenwallet.UTXO{mockUTXO})
	if err != nil {
		return 0, err
	}
	...
	mockTxWithoutUTXO, err := libkarlsenwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, nil)
	if err != nil {
		return 0, err
