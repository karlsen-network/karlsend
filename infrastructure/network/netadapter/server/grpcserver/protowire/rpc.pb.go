...
//
// Clients are expected to build RequestMessages and wrap them in KarlsendMessage.
// (see messages.proto)
//
// Having received a RequestMessage, (wrapped in a KarlsendMessage) the RPC server
// will respond with a ResponseMessage (likewise wrapped in a KarlsendMessage)
...
// GetCurrentNetworkRequestMessage requests the network karlsend is currently
// running against.
//
...
type GetBlockTemplateResponseMessage struct {
	...
	// Callers are discouraged (but not forbidden) from solving blocks when karlsend
	// is not synced. That is because when karlsend isn't in sync with the rest of
...
// GetPeerAddressesRequestMessage requests the list of known karlsend addresses in
...
// peers currently connected to this karlsend.
...
// selected parent chain from some startHash to this karlsend's current virtual
...
// this karlsend's current virtual.
...
// karlsend. Note that this number may decrease as pruning occurs.
